...
Установите и авторизуйте официальный Gemini CLI, затем задайте `GEMINI_API_KEY`
(`GEMINI_KEY` также поддерживается). Пункт `AI` создаёт тип и техническую тему до
72 символов только по `git diff --cached` (до 512 KiB). Проверьте stage на секреты;
хуки Git/Husky выполняются как обычно.
